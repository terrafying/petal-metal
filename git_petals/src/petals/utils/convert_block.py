...
import torch
# On macOS MPS, tensor_parallel may call torch.mps.current_device, which doesn't exist by default.
# Provide a stub so that TP works with a single MPS device (index 0).
if getattr(torch.backends, 'mps', None) and torch.backends.mps.is_available():
    if not hasattr(torch.mps, 'current_device'):
        def current_device() -> int:
            return 0
        setattr(torch.mps, 'current_device', current_device)
